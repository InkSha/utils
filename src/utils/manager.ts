 ...
 * 管理器
 */
export class Manager<Element = number> {
  private id: number[] = []
  private values: Map<number, Element> = new Map()
   ...
   */
  load<T extends Element>(
    initialization: T[],
    callback?: (id: number, val: T) => void,
  ) {
   ...
   * @returns 获取的成员
   */
  get<T extends Element>(id: number) {
    return this.has(id) ? (this.values.get(id) as T) : null
  }
   ...
   * @returns 删除的成员
   */
  del<T extends Element>(id: number) {
   ...
   * @returns 更改的成员 id
   */
  update<T extends Element>(id: number, val: T) {
    this.values.set(id, val)
    return id
  }
